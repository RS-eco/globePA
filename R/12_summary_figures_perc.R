...
p1 <- ter_dat %>% filter(path=="Annual mean temp. (°C)") %>% ggplot() + 
  ...
  geom_text(data=test %>% filter(path=="Annual mean temp. (°C)"), aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
...
p
ggsave("figures/Figure2_perc.png", p, dpi=1000, width=10, height=7)
ggsave("figures/Figure2.pdf", p, dpi=1000, width=10, height=7)
