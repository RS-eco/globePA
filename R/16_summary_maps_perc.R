...
prot_all_long <- prot_all %>% tidyr::drop_na() %>% gather(var, value, -c(x,y)) %>%
...
p1 <- ggplot() + geom_tile(data=prot_all_long, aes(x=x, y=y, fill=perc2)) + 
  facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
  geom_sf(data=outline_ter_moll, fill=NA, na.rm=T, color = "black", size = 0.25/.pt) +
  coord_sf() + theme_bw() + 
...
# Save figure
ggsave("figures/Figure4_perc.png", p1, width=9, height=6, dpi=1000)
ggsave("figures/Figure4.pdf", p1, width=9, height=6, dpi=1000)

ter_dat_ind %>% group_by(path) %>% summarise(sum(n))
ter_dat_ind %>% mutate(perc2 = as.character(cut(Total, breaks=round(c(0,10^(seq(0,2,0.2))),0), include.lowest=T,
                                                labels=c("0 - 1", "1 - 2", "2 - 3", "3 - 4", "4 - 6", "6 - 10", "10 - 16", "16 - 25", "25 - 40", "40 - 63", "63 - 100")))) %>% drop_na() %>%
...
prot_all_long <- prot_all %>% tidyr::drop_na() %>% gather(var, value, -c(x,y)) %>%
  #mutate(perc2 = as.character(cut(value, breaks=seq(0,10,1)^2, include.lowest=T,
...
#library(classInt)
#classIntervals(prot_all_long$value, style="sd", n=9)

# Plot maps
p2 <- ggplot() + geom_tile(data=prot_all_long, aes(x=x, y=y, fill=perc2)) + 
  facet_wrap(var~., nrow=3) + 
  scale_fill_manual(values = redblue, name = "% Protected", na.value="transparent",
                    guide = guide_legend(reverse = TRUE)) + 
...
# Save figure
ggsave("figures/Figure5_perc.png", p2, width=7.8, height=6, dpi=1000)
ggsave("figures/Figure5.pdf", p2, width=7.8, height=6, dpi=1000)

marspec_dat_ind %>% group_by(path) %>% summarise(sum(n))
marspec_dat_ind %>% 
  mutate(perc2 = as.character(cut(Total, breaks=round(c(0,10^(seq(0,2,0.2))),0), include.lowest=T,
                                  labels=c("0 - 1", "1 - 2", "2 - 3", "3 - 4", "4 - 6", "6 - 10", "10 - 16", "16 - 25", "25 - 40", "40 - 63", "63 - 100")))) %>% drop_na() %>%
  mutate(perc2 = factor(if_else(Total == 0, "0", perc2),
                        levels=c("0", "0 - 1", "1 - 2", "2 - 3", "3 - 4", "4 - 6", "6 - 10", "10 - 16", "16 - 25", "25 - 40", "40 - 63", "63 - 100"))) %>% 
