...
p1 <- ter_dat %>% filter(path=="Annual mean temp. (°C)") %>% ggplot() + 
  ...
  facet_grid(. ~ path, switch="x", scales="free_x") + 
  geom_text(data=test %>% filter(path=="Annual mean temp. (°C)"), 
            aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
  labs(y="Area protected (%)") + # Annual mean temperature is too long
  scale_x_continuous(breaks=c(2,26,50,75,99), 
                     labels=as.vector(ter_dat %>% filter(path=="Annual mean temp. (°C)") %>% 
                                        ungroup() %>% select("var2") %>% unlist(use.names=F))[c(2,26,50,75,99)], 
  ...
  #annotate("text", x = 50, y = Inf,  label=as.character(paste(expression(italic(chi)^2), " ==",
  #                                                            test %>% filter(path=="Annual mean temp. (°C)") %>% 
  #                                                              select(chisq) %>% unlist())), 
  #         vjust = +1.5, parse=T) + 
  #annotate("text", x = 50, y = Inf,  label=paste("df ==", 
  #                                               test %>% filter(path=="Annual mean temp. (°C)") %>% 
  #                                                 select(df) %>% unlist()), 
  #         vjust = +4, parse=T) + 
  #annotate("text", x = 50, y = Inf,  label=as.character(paste(expression(italic(p)), " ==", 
  #                                               test %>% filter(path=="Annual mean temp. (°C)") %>% 
  #                                                 select(pvalue) %>% unlist())), 
  #         vjust = +4.75, parse=T) + 
...
ter_dat %>% filter(path=="Annual mean temp. (°C)") %>% 
  group_by(x,y) %>% summarise(perc=sum(perc), exp=mean(exp)) %>% 
  filter(perc < exp) %>% view()

p2 <- ter_dat %>% filter(path=="Annual precipitation (mm)") %>% ggplot() + 
  geom_bar(aes(x = var, y=perc, fill=iucn_cat), width=1, stat="identity", position="stack") +
  geom_line(aes(x=var, y=exp), colour="black") + 
  facet_grid(. ~ path, switch="x", scales="free_x") + 
  geom_text(data=test %>% filter(path=="Annual precipitation (mm)"), aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
  #geom_text(data=test %>% filter(path=="Annual precipitation (mm)"), aes(x = 50, y = Inf,  label=paste(expression(italic(chi)^2), " ==", chisq)), vjust = +1.5, parse=T) + 
  #geom_text(data=test %>% filter(path=="Annual precipitation (mm)"), aes(x = 50, y = Inf,  label=paste("df = ", df)), vjust = +4) + 
  #geom_text(data=test %>% filter(path=="Annual precipitation (mm)"), aes(x = 50, y = Inf,  label=paste(expression(italic(p))," ==", pvalue)), vjust = +4.75, parse=T) + 
  scale_x_continuous(breaks=c(2,26,50,75,99), 
                     labels=round(as.vector(ter_dat %>% filter(path=="Annual precipitation (mm)") %>% ungroup() %>% select("var2") %>% unlist(use.names=F)),0)[c(2,26,50,75,99)], 
                     limits=c(0,101), expand=c(0,0)) + 
  scale_y_continuous(expand=expansion(mult=c(0,.01)), limits=c(0,48)) + # , breaks=c(0, 1, 5, 15, 30)) +
  scale_fill_manual(values=c("#D43F3AFF", "#EEA236FF", "#46B8DAFF", "#5CB85CFF"), 
                    guide = guide_legend(reverse = TRUE)) + 
...
ter_dat %>% filter(path=="Annual precipitation (mm)") %>% 
  group_by(x,y) %>% summarise(perc=sum(perc), exp=mean(exp)) %>% 
  filter(perc < exp) %>% view()

p3 <- ter_dat %>% filter(path=="Elevation (m)") %>% ggplot() + 
  geom_bar(aes(x = var, y=perc, fill=iucn_cat), width=1, stat="identity", position="stack") +
  geom_line(aes(x=var, y=exp), colour="black") + 
  facet_grid(. ~ path, switch="x", scales="free_x") + 
  geom_text(data=test %>% filter(path=="Elevation (m)"), aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
  #geom_text(data=test %>% filter(path=="Elevation (m)"), aes(x = 50, y = Inf,  label=paste(expression(italic(chi)^2), " ==", chisq)), vjust = +1.5, parse=T) + 
  #geom_text(data=test %>% filter(path=="Elevation (m)"), aes(x = 50, y = Inf,  label=paste("df = ", df)), vjust = +4) + 
  #geom_text(data=test %>% filter(path=="Elevation (m)"), aes(x = 50, y = Inf,  label=paste(expression(italic(p))," ==", pvalue)), vjust = +4.75, parse=T) + 
  scale_x_continuous(breaks=c(2,26,50,75,99), 
...
ter_dat %>% filter(path=="Elevation (m)") %>% group_by(x,y) %>% summarise(perc=sum(perc), exp=mean(exp)) %>%
  filter(perc < exp) %>% view()
...
m_ee1 <- m_ee %>% filter(path != "bathy_perc")
m_ee2 <- m_ee %>% filter(path == "bathy_perc")
m_ee1$var2 <- round(rowMeans(m_ee1[, c("x", "y")])/100,1)
m_ee2$var2 <- round(rowMeans(m_ee2[, c("x", "y")]),0)
m_ee <- bind_rows(m_ee1, m_ee2)

marspec_dat %<>% filter(path %in% c("biogeo13_perc", "biogeo08_perc", "bathy_perc")) %>% left_join(exp_val) %>% 
  left_join(as.data.frame(m_ee)) %>% 
  mutate(path = factor(path, levels =  c("biogeo13_perc", "biogeo08_perc", "bathy_perc"),
                       labels = c("Mean annual SST (°C)", "Mean annual SSS (psu)", "Bathymetry (m)")))
...
p4 <- marspec_dat %>% filter(path=="Mean annual SST (°C)") %>%
  ggplot() + geom_bar(aes(x = var, y=perc, fill=iucn_cat), width=1, stat="identity", position="stack") +
  geom_line(aes(x=var, y=exp), colour="black") + 
  facet_grid(. ~ path, switch="x", scales="free_x") + 
  geom_text(data=test %>% filter(path=="Mean annual SST (°C)"), aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
  #geom_text(data=test %>% filter(path=="Mean annual SST (°C)"), aes(x = 48, y = Inf,  label=paste(expression(italic(chi)^2), " ==", chisq)), vjust = +1.5, parse=T) + 
  #geom_text(data=test %>% filter(path=="Mean annual SST (°C)"), aes(x = 48, y = Inf,  label=paste("df = ", df)), vjust = +4) + 
  #geom_text(data=test %>% filter(path=="Mean annual SST (°C)"), aes(x = 48, y = Inf,  label=paste(expression(italic(p))," ==", pvalue)), vjust = +4.75, parse=T) + 
  ...
  scale_y_continuous(expand=expansion(mult=c(0,.01)), limits=c(0,23)) + # , breaks=c(0, 1, 5, 15, 30)) +
...
marspec_dat %>% filter(path=="Mean annual SST (°C)") %>% group_by(x,y) %>% summarise(perc=sum(perc), exp=mean(exp))

p5 <- marspec_dat %>% filter(path=="Mean annual SSS (psu)") %>%
  ggplot() + geom_bar(aes(x = var, y=perc, fill=iucn_cat), width=1, stat="identity", position="stack") +
  geom_line(aes(x=var, y=exp), colour="black") + 
  facet_grid(. ~ path, switch="x", scales="free_x") + 
  geom_text(data=test %>% filter(path=="Mean annual SSS (psu)"), aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
  #geom_text(data=test %>% filter(path=="Mean annual SSS (psu)"), aes(x = 42, y = Inf,  label=paste(expression(italic(chi)^2), " ==", chisq)), vjust = +1.5, parse=T) + 
  #geom_text(data=test %>% filter(path=="Mean annual SSS (psu)"), aes(x = 42, y = Inf,  label=paste("df = ", df)), vjust = +4) + 
  #geom_text(data=test %>% filter(path=="Mean annual SSS (psu)"), aes(x = 42, y = Inf,  label=paste(expression(italic(p))," ==", pvalue)), vjust = +4.75, parse=T) + 
...
marspec_dat %>% filter(path=="Mean annual SSS (psu)") %>% group_by(x,y) %>% summarise(perc=sum(perc), exp=mean(exp)) %>% 
  filter(perc < exp) %>% view()

p6 <- marspec_dat %>% filter(path=="Bathymetry (m)") %>% 
  ggplot() + geom_bar(aes(x = var, y=perc, fill=iucn_cat), width=1, stat="identity", position="stack") +
  geom_line(aes(x=var, y=exp), colour="black") + 
  facet_grid(. ~ path, switch="x", scales="free_x") + 
  geom_text(data=test %>% filter(path=="Bathymetry (m)"), aes(x = -Inf, y = Inf, label=letter), hjust   = -0.2, vjust   = +1.5) + 
  #geom_text(data=test %>% filter(path=="Bathymetry (m)"), aes(x = 50, y = Inf,  label=paste(expression(italic(chi)^2), " ==", chisq)), vjust = +1.5, parse=T) + 
  #geom_text(data=test %>% filter(path=="Bathymetry (m)"), aes(x = 50, y = Inf,  label=paste("df = ", df)), vjust = +4) + 
  #geom_text(data=test %>% filter(path=="Bathymetry (m)"), aes(x = 50, y = Inf,  label=paste(expression(italic(p))," ==", pvalue)), vjust = +4.75, parse=T) + 
  scale_x_continuous(breaks=c(2,26,50,75,99), 
                     labels=as.vector(marspec_dat %>% filter(path=="Bathymetry (m)") %>%
                                        ungroup() %>% select("var2") %>% unlist(use.names=F))[c(2,26,50,75,99)], 
                     limits=c(0,101), expand=c(0,0)) + 
  scale_y_continuous(expand=expansion(mult=c(0,.01)), limits=c(0,23)) + #, breaks=c(0, 1, 5, 15, 30)) +
  scale_fill_manual(values=c("#D43F3AFF", "#EEA236FF", "#46B8DAFF", "#5CB85CFF"), 
...
marspec_dat %>% filter(path=="Bathymetry (m)") %>%
  group_by(x,y) %>% summarise(perc=sum(perc), exp=mean(exp)) %>% 
  filter(perc < exp) %>% view()

p7 <- ggpubr::as_ggplot(ggpubr::get_legend(p3))
ggsave("figures/Figure2_perc_legend.png", p7, dpi=1000, width=4, height=0.5)

p <- p1 + p2 + {p3 + theme(legend.position="none")} + p4 + p5 + p6 + plot_spacer() + p7 + plot_spacer() + 
  plot_layout(ncol=3, heights=c(6,6,1))
p
ggsave("figures/Figure2_perc_no-test.png", p, dpi=1000, width=10, height=7)
